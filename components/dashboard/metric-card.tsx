...
interface Props { href: string; title: string; value: string; detail: string; trend: string; icon: LucideIcon; positive?: boolean }
export function MetricCard({ href, title, value, detail, trend, icon: Icon, positive = true }: Props) {
  const Trend = positive ? ArrowUpRight : ArrowDownRight
  return (
    <Link href={href} className="group block rounded-2xl focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-blue-500/30">
      <Card className="transition-transform transition-shadow group-hover:-translate-y-0.5 group-hover:shadow-md">
