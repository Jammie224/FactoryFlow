...
const navigationItems = [
  { label: "儀表板", icon: Gauge, active: true },
  { label: "生產管理", icon: Boxes },
  { label: "日產報表", icon: FileText },
  { label: "統計分析", icon: BarChart3 },
  { label: "匯出報表", icon: Download },
  { label: "使用者管理", icon: Users },
  { label: "系統設定", icon: Settings },
]
    ...
    <aside className="hidden w-64 shrink-0 flex-col border-r border-blue-900 bg-[#0c2855] px-4 py-5 text-white lg:flex">
      ...
      <nav className="space-y-1" aria-label="主要導覽">
        {navigationItems.map((item) => (
          <a key={item.label} href="#" aria-current={item.active ? "page" : undefined} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition", item.active ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5 hover:text-white")}>
            <item.icon className={cn("size-[18px]", item.active && "text-blue-300")} />
            {item.label}
